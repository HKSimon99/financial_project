...
async def get_logo_cached(naver: NaverImageSearch, *, company_name: str, stock_code: str | None = None) -> dict:
    ...
    if not (naver and naver.client_id and naver.client_secret):
        data = {"company": company_name, "stock_code": stock_code, "logo_url": None, "cached": False, "reason": "missing_naver_credentials"}
    ...
    data = {"company": company_name, "stock_code": stock_code, "logo_url": url, "cached": False}
    save_json(data, cache_file)
    return data
