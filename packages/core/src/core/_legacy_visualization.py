...
def _update_common_layout(fig: go.Figure, title: str, dark_mode: bool = False):
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(0,0,0,0)' if dark_mode else 'rgba(0,0,0,0)', # 배경 투명하게
        plot_bgcolor='rgba(0,0,0,0)' if dark_mode else 'rgba(0,0,0,0)' # 플롯 영역 배경 투명하게
    )
    # 축 색상 설정
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(150,150,150,0.2)' if dark_mode else 'rgba(200,200,200,0.5)', 
                     zeroline=False, linecolor='rgba(150,150,150,0.5)' if dark_mode else 'rgba(100,100,100,0.5)', 
                     tickfont=dict(color="white" if dark_mode else "black"), 
                     title_font=dict(color="white" if dark_mode else "black"))
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(150,150,150,0.2)' if dark_mode else 'rgba(200,200,200,0.5)',
                     zeroline=False, linecolor='rgba(150,150,150,0.5)' if dark_mode else 'rgba(100,100,100,0.5)',
                     tickfont=dict(color="white" if dark_mode else "black"),
                     title_font=dict(color="white" if dark_mode else "black"))
...
# 재무 비율 막대그래프
# ---------------------------------------------------
def plot_ratios(health_dict: dict, title: str = "주요 재무 비율", dark_mode: bool = False) -> go.Figure:
    """
    ...
    if not health_dict or all(pd.isna(v) for v in health_dict.values()):
        logging.warning("재무 건전성 데이터가 비어있어 재무 비율 차트를 생성할 수 없습니다.")
        fig = go.Figure()
...
# ---------------------------------------------------
def plot_price(price_df: pd.DataFrame, corp_name: str, chart_type: str = "line", dark_mode: bool = False) -> go.Figure:
    """
    ...
    if price_df.empty or 'date' not in price_df.columns:
        logging.warning(f"{corp_name} 주가 데이터가 비어있거나 'date' 컬럼이 없어 차트를 생성할 수 없습니다.")
        fig = go.Figure()
    ...
    if chart_type == "candlestick":
        ...
        if not all(col in price_df.columns for col in required_cols):
            logging.warning(f"캔들스틱 차트 생성에 필요한 컬럼({required_cols})이 부족합니다. 라인 차트로 대체합니다.")
        ...
        if chart_type == "candlestick":
            ...
            fig.update_layout(xaxis_rangeslider_visible=False) # 캔들스틱 기본 슬라이더 숨김
...
# 업종별 평균 지표 막대그래프
# ---------------------------------------------------
def plot_industry_avg(industry_avg_df: pd.DataFrame, title: str = "업종별 평균 총점", dark_mode: bool = False) -> go.Figure:
    """
    ...
    Returns:
        go.Figure: Plotly Figure 객체.
    """
    if industry_avg_df.empty or "업종" not in industry_avg_df.columns or "total_score" not in industry_avg_df.columns:
        logging.warning("업종별 평균 데이터가 비어있거나 필수 컬럼이 없어 업종 평균 차트를 생성할 수 없습니다.")
        fig = go.Figure()
        _update_common_layout(fig, f"{title} (데이터 없음)", dark_mode)
        return fig
...
def plot_company_vs_industry_avg(
    ...
    Returns:
        go.Figure: Plotly Figure 객체.
    """
    if company_data.empty or metric_name not in company_data.index or pd.isna(company_data[metric_name]):
        logging.warning(f"기업 '{company_name}'의 '{metric_name}' 데이터가 유효하지 않아 비교 차트를 생성할 수 없습니다.")
        fig = go.Figure()
        _update_common_layout(fig, f"'{company_name}' vs 업종 평균 ({metric_name}) (데이터 없음)", dark_mode)
        return fig
    ...
    if pd.isna(industry_avg_value):
        logging.warning(f"업종 '{company_industry}'의 '{metric_name}' 평균 데이터를 찾을 수 없어 비교 차트를 생성할 수 없습니다.")
        fig = go.Figure()
        _update_common_layout(fig, f"'{company_name}' vs 업종 평균 ({metric_name}) (업종 평균 데이터 없음)", dark_mode)
        return fig
    ...
    fig = px.bar(
        ...
        color_discrete_map={
            ...
            f'{company_industry} 평균': px.colors.qualitative.Pastel[1] # 업종 평균 색상
        }
    )
    fig.update_traces(texttemplate='%{text:.2f}', textposition="outside",
                      marker_line_color='rgb(8,48,107)', marker_line_width=1.5, opacity=0.8)
    ...
    _update_common_layout(fig, f"'{company_name}' ({company_industry}) vs 업종 평균: {metric_name}", dark_mode)
    logging.info(f"✅ 기업 vs 업종 평균 비교 차트 생성 완료: {company_name} - {metric_name}")
    return fig