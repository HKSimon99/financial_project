...
from ._legacy_data_fetch import load_or_create_corp_code_list, get_fiscal_month # get_fiscal_month는 현재 사용되지 않지만, 혹시 모를 미래 확장을 위해 유지
...
if not API_KEY:
    logging.warning("API_KEY가 설정되지 않았습니다. .env 파일을 확인하거나 data_fetch 모듈이 먼저 실행되었는지 확인하세요.")
...
class PDFReport(FPDF):
    ...
    def add_korean_font(self):
        ...
        font_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'fonts', 'NotoSansKR-Regular.ttf')
        ...
        if os.path.exists(font_path):
            try:
                ...
                logging.error(f"❌ 한글 폰트 로드 실패 ({font_path}): {e}. Arial로 대체됩니다.", exc_info=True)
                self.set_font("Arial", "", 12)
        else:
            logging.warning(f"❌ 한글 폰트 파일이 없습니다: {font_path}. Arial로 대체됩니다. fonts/NotoSansKR-Regular.ttf를 추가해주세요.")
...
def _get_corp_codes_info(corp_name: str) -> tuple[str, str]:
    ...
    stock_code = str(row['stock_code']).zfill(6) if pd.notna(row['stock_code']) else None
...
def _fetch_company_overview_safe(corp_code: str) -> dict:
    ...
    try:
        ...
        if data.get("status") == "000":
            ...
            logging.warning(f"DART 기업 개요 조회 실패 ({corp_code}): {data.get('message', '알 수 없는 오류')}")
            return {}
    except requests.exceptions.RequestException as e:
        logging.error(f"DART 기업 개요 API 요청 오류 ({corp_code}): {e}", exc_info=True)
        return {}
    except Exception as e:
        logging.error(f"기업 개요 처리 중 예상치 못한 오류 발생 ({corp_code}): {e}", exc_info=True)
        return {}
...
def _fetch_company_logo_safe(stock_code: str) -> BytesIO | None:
    ...
    if not stock_code:
        return None
    try:
        logo_url = f"https://ssl.pstatic.net/imgfinance/chart/item/200x200/{stock_code}.png"
        ...
        if res.status_code == 200 and res.content:
            ...
            logging.warning(f"기업 로고를 찾을 수 없거나 응답이 비어있습니다: {stock_code}")
            return None
    except requests.exceptions.RequestException as e:
        logging.warning(f"기업 로고 다운로드 실패 ({stock_code}): {e}")
        return None
    except Exception as e:
        logging.error(f"기업 로고 처리 중 예상치 못한 오류 발생 ({stock_code}): {e}", exc_info=True)
        return None
...
def _create_price_chart(price_df: pd.DataFrame, corp_name: str) -> BytesIO | None:
    ...
    if price_df.empty or 'date' not in price_df.columns or 'close' not in price_df.columns:
        logging.warning(f"주가 데이터가 불완전하여 주가 차트를 생성할 수 없습니다: {corp_name}")
        return None
...
def _create_ratio_chart(health: dict, corp_name: str, year: int) -> BytesIO | None:
    ...
    if not health or all(pd.isna(v) for v in health.values()):
        logging.warning(f"재무 건전성 데이터가 없어 재무비율 차트를 생성할 수 없습니다: {corp_name}")
        return None

    # 차트 데이터 준비
    ratio_names = ["ROE", "부채비율", "유동비율", "영업이익률", "이자보상배율", "Z-score"]
    ratio_values = [
    ...
    filtered_ratios = [(name, value) for name, value in zip(ratio_names, ratio_values) if not pd.isna(value)]
    if not filtered_ratios:
        logging.warning(f"유효한 재무비율 데이터가 없어 재무비율 차트를 생성할 수 없습니다: {corp_name}")
        return None
    ...
    try:
        ...
        logging.error(f"재무비율 차트 이미지 생성 실패 ({corp_name}): {e}", exc_info=True)
        return None

# ---------------------------------------------------
# Excel 보고서 저장 (기존 기능 유지)
# ---------------------------------------------------
def save_excel_report(df_results: pd.DataFrame, df_industry_avg: pd.DataFrame, filename: str = "financial_report.xlsx") -> str:
    """
...
# 통합 투자 리포트 생성 (PDF)
# ---------------------------------------------------
def generate_investment_report(corp_name: str, year: int, health: dict, fs_df: pd.DataFrame, price_df: pd.DataFrame) -> BytesIO | None:
    """
    ...
    try:
        ...
        pdf.cell(0, 15, pdf.safe_text(f"{corp_name} {year} 투자 리포트"), ln=True, align='C')
        pdf.ln(5)
        ...
        total_score_str = f"{health['total_score']:.2f}" if not pd.isna(health.get('total_score')) else "N/A"
        grade_str = health.get('grade', 'N/A')
        pdf.cell(0, 10, pdf.safe_text(f"✨ 종합 점수: {total_score_str} | 등급: {grade_str}"), ln=True)
        pdf.ln(5)
        ...
        if fs_summary:
            ...
            for k, v in fs_summary.items():
                if not pd.isna(v):
                    pdf.cell(0, 7, pdf.safe_text(f"{k}: {v:,.0f} 원"), ln=True) # 천단위 콤마, 원 표시
            pdf.ln(5)
        ...
        if price_chart_bytes:
            if pdf.get_y() > pdf.h - 80: # 현재 페이지 하단에 공간이 부족하면 새 페이지 시작
                pdf.add_page()
            pdf.set_font("CustomFont", 'B', 14)
